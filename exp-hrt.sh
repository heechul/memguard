...
do_hrt_test()
{
	...
	./hrt -c 2 -i 1000 -C 12 -I 10 $master_schedopt > $TMPFILE \
	|| error "exec failed"
    killall -2 thr hrt bandwidth latency cpuhog matrix
    killall -9 cpuhog
    print_settings
    sleep 1
    ./printstat.py --deadline=13 $TMPFILE >> $outputfile
...
#    do_hrt_test co-run
    ...
    do_init_mb "100 200 900" 1 2  >& /dev/null
    do_hrt_test solo "-o fifo"  "-o normal"
    #do_hrt_test co-run "-o fifo"  "-o normal"

    log_echo ">> memguard(excl5)"
    do_init_mb "100 200 900" 1 5  >& /dev/null
    do_hrt_test solo "-o fifo"  "-o normal"
